...
int SetIndent(xmlTextWriterPtr writer, int indent) {
  xmlTextWriterSetIndent(writer, indent);
  if (indent) {
    int i;
    xmlChar * str = (xmlChar *)malloc(indent+1);
    for (i=0; i<indent; i++) str[i] = ' ';
    str[indent] = '\0';
    xmlTextWriterSetIndentString(writer, str);
  }
  return 0;
}

int SetTestOpts(xmlTextWriterPtr writer) {
    if (indent) SetIndent(writer, indent);
    if (view) xmlTextWriterSetView(writer, view);
    return 0;
}

int usage() {
  printf("%s\n", "\
Usage: testWriter [OPTIONS]\n\
\n\
Options:\n\
  -?            Display this help screen\n\
  -i INDENT	Indent the output with INDENT spaces\n\
  -s            Output SML\n\
  -x            Output XML\n\
");
  return 0;
