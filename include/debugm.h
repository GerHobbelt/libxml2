...
static int iDebug0 = 0;	/* Global variable enabling debug output if TRUE. */   \
...
DEBUG_GLOBAL_VARS							       \
int * get_iDebug(void) {return &iDebug0;}				       \
...
#if !defined(_MSDOS)
XMLPUBFUN int  XMLCALL debug_printf(char *fmt,...);	/* Print debug messages */
XMLPUBFUN char * XMLCALL debug_sprintf(char *fmt,...); /* Print debug messages to a new buffer */
XMLPUBFUN char * XMLCALL debug_vsprintf(char *fmt, va_list vl); /* Common subroutine of the previous two */
#endif /* !defined(_MSDOS) */
